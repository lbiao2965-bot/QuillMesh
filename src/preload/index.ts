...
export interface ElectronAPI {
  ...
  chooseBibliography: (documentId: string) => Promise<{ path: string; content: string } | null>
  findBibliographyForDocument: (documentId: string) => Promise<{ path: string; content: string } | null>
  appendBibliographyEntry: (documentId: string, text: string) => Promise<boolean>
  ...
  saveImageBytes: (bytes: Uint8Array, mime: string, suggestedName: string) => Promise<boolean>
...
const api: ElectronAPI = {
  ...
  chooseBibliography: (documentId) => ipcRenderer.invoke('choose-bibliography', documentId),
  findBibliographyForDocument: (documentId) => ipcRenderer.invoke('find-bibliography-for-document', documentId),
  appendBibliographyEntry: (documentId, text) => ipcRenderer.invoke('append-bibliography-entry', documentId, text),
  ...
  saveImageBytes: (bytes, mime, suggestedName) => ipcRenderer.invoke('save-image-bytes', bytes, mime, suggestedName),
